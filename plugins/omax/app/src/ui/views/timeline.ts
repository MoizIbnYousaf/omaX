...
export class TimelineView implements OmaXView {
  ...
  private feedType: FeedType = "foryou";
  ...
  public render(): ViewDescriptor {
    const feedLabel = this.feedType === "foryou" ? "For You" : "Following";
    const title = `Home Timeline (${feedLabel})`;

    if (this.items.length === 0 && this.loading) {
      return {
        title,
        hints: "Tab: switch feed | j/k: navigate | q: back",
        content: Box(
          {
            width: "100%",
            height: "100%",
    ...
    if (this.items.length === 0) {
      return {
        title,
        hints: "Tab: switch feed | n: load | q: back",
        content: Box(
          {
            width: "100%",
            height: "100%",
    ...
    return {
      title,
      hints:
        "Tab: switch feed | j/k: navigate | l: like | b: bookmark | r: reply | Enter: open | p: profile | c: compose | n: more | q: back",
      content: Box(
        {
          width: "100%",
          height: "100%",
  ...
  private async switchFeed(): Promise<void> {
    this.feedType = this.feedType === "foryou" ? "following" : "foryou";
    this.items = [];
  ...
  private async loadMore(): Promise<void> {
    ...
    this.loading = true;
    const feedLabel = this.feedType === "foryou" ? "For You" : "Following";
    this.ctx.setStatus(`Loading ${feedLabel} timeline...`);

    try {
      const count = 20;
      const result =
        this.feedType === "foryou"
          ? await this.ctx.client.getHomeTimeline(count)
          : await this.ctx.client.getHomeLatestTimeline(count);

      if (!result.success) {
        this.ctx.setStatus(`Timeline error: ${result.error}`);
        return;
      }
      ...
      this.ctx.setStatus(`Loaded ${items.length} posts.`);
    } catch (error) {
      this.ctx.setStatus(
        `Timeline request failed: ${(error as Error).message}`,
      );
    } finally {
      this.loading = false;
    }
