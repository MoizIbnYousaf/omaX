...
export class ListTimelineView implements OmaXView {
  ...
  public constructor(ctx: ViewContext, listId: string, listName: string) {
    this.ctx = ctx;
    this.listId = listId;
    this.listName = listName;
    this.viewId = `list-timeline-${listId}`;
    this.scrollId = `list-timeline-scroll-${listId}`;
  }
  ...
  public render(): ViewDescriptor {
    const title = `List: ${this.listName}`;

    if (this.items.length === 0 && this.loading) {
      return {
        title,
        hints: "j/k: navigate | q: back",
        content: Box(
          {
            width: "100%",
            height: "100%",
          ...
          Text({ content: "Loading list timeline...", fg: theme.textMuted }),
        ),
      };
    }
    ...
    if (this.items.length === 0) {
      return {
        ...
        content: Box(
          ...
          Text({ content: "No posts in this list.", fg: theme.textMuted }),
        ),
      };
    }
    ...
    return {
      title,
      hints:
        "j/k: navigate | l: like | b: bookmark | r: reply | Enter: open | p: profile | n: more | q: back",
      content: Box(
        {
          width: "100%",
          height: "100%",
  ...
  private async loadMore(): Promise<void> {
    ...
    this.loading = true;
    this.ctx.setStatus(`Loading list timeline...`);

    try {
      const count = 20;
      const result = await this.ctx.client.getListTimeline(this.listId, count);

      if (!result.success) {
        this.ctx.setStatus(`List timeline error: ${result.error}`);
        return;
      }
      ...
      this.ctx.setStatus(
        `List timeline request failed: ${(error as Error).message}`,
      );
    } finally {
      this.loading = false;
    }
