...
function resolveFeaturesCachePath(): string {
  const override = process.env.OMAX_FEATURES_CACHE ?? process.env.OMAX_FEATURES_PATH;
  if (override && override.trim().length > 0) {
    return path.resolve(override.trim());
  }
  return path.join(homedir(), '.config', 'omax', DEFAULT_CACHE_FILENAME);
}
...
function readOverridesFromEnv(): NormalizedFeatureOverrides | null {
  const raw = process.env.OMAX_FEATURES_JSON;
