...
import { getInlineImageData, type InlineImageData } from "./post-image-preview.js";
...
export class InlineImageManager {
  ...
  public constructor(
    ...
    private readonly configuredMode: ImageMode,
  ...
  public async reconcileMany(desiredStates: DesiredInlineImageState[]): Promise<ResolvedImageMode> {
    ...
    // Phase 1 — kick off every fetch in parallel up front so we never serialize
    // network/decoding work. Cached entries resolve synchronously on the next
    // microtask, so the whole batch lands quickly on subsequent reconciles.
    interface FetchTask {
      desired: DesiredInlineImageState;
      imageId: string;
      placement: InlineImagePlacement;
      assetPromise: Promise<InlineImageData | undefined>;
    }
    ...
    for (const desired of desiredStates) {
      ...
      const assetPromise = getInlineImageData(desired.imageUrl, {
      ...
      fetchTasks.push({ desired, imageId, placement, assetPromise });
    }

    // Phase 2 — as each fetch resolves, draw it. Backend writes are still
    // serialized inside the kitty backend so the cell never tears.
    for (const task of fetchTasks) {
      const asset = await task.assetPromise;

      if (sequence !== this.reconcileSequence) {
        return mode;
      }
      if (!asset) {
        await this.hideKittyImageById(task.imageId);
        continue;
      }

      const displayPlacement = this.computeDisplayPlacementForAsset(task.placement, asset.width, asset.height);
      const request: InlineImageRequest = {
        imageId: task.imageId,
        imageKey: `${task.desired.imageUrl}::${asset.width}x${asset.height}`,
      ...
      const active = this.activeKittyImages.get(task.imageId);
      ...
      if (sequence !== this.reconcileSequence) {
        await this.kittyBackend.hide(task.imageId);
        return mode;
      }

      this.activeKittyImages.set(task.imageId, {
        imageId: task.imageId,
        imageKey: request.imageKey,
        placementKey,
        viewId: task.desired.viewId,
      });
    }
