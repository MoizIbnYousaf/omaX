...
export class ProfileJumpView implements OmaXView {
  ...
  private async submit(value: string): Promise<void> {
    ...
    try {
      const lookup = await this.ctx.client.getUserIdByUsername(username);
      if (!lookup.success) {
        this.ctx.setStatus(lookup.error ?? `Could not find @${username}.`);
        this.ctx.popView();
        return;
      }
      this.ctx.popView();
